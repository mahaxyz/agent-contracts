...
contract AchievementNFT is IAchievementNFT, OwnableUpgradeable, ERC721EnumerableUpgradeable {
  ...
  function initialize(string memory name_, string memory symbol_, address backend) external initializer {
  ...
  //@inheritdoc IAchievementNFT
  function mintAchievement(address to, Achievement memory achievement) external onlyMinter returns (uint256) {
  ...
  {
    require(recipients.length == achievements.length, "AchievementNFT: array length mismatch");

    uint256[] memory tokenIds = new uint256[](recipients.length);

    for (uint256 i = 0; i < recipients.length; i++) {
      tokenIds[i] = _mintSingleAchievement(recipients[i], achievements[i]);
    }
  ...
  function _mintSingleAchievement(address to, Achievement memory achievement) internal returns (uint256) {
