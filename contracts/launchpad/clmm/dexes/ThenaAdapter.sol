...
contract ThenaAdapter is BaseV3Adapter {
    ...
    function _collectFees(
        uint256 _lockId
    ) internal override returns (uint256 fee0, uint256 fee1) {
        (fee0, fee1, , ) = IGoPlusLocker(locker).collect(
            _lockId,
            address(this),
            type(uint128).max,
            type(uint128).max
    ...
    function _createPool(
        ...
        IERC20 _token1,
        uint24 _fee,
        uint160 _sqrtPriceX96Launch
    ) internal virtual override returns (IClPool pool) {
        address _pool = INonfungiblePositionManagerThena(
            address(nftPositionManager)
